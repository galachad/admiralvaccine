...
testthat::test_that("derive_vars_merged_vaccine test case 1- merge the ex with face on records where FASCAT='SYSTEMIC'  ",{
  ...
  expect_sys <- face1 %>% mutate(FATPTREF='VACCINATION 1') %>%
    left_join(ex1 %>% mutate(EXTPTREF='VACCINATION 1'), by=c('USUBJID'='USUBJID','FATPTREF'='EXTPTREF')) %>% filter(FASCAT=='SYSTEMIC')
  ...
  expected <- plyr::rbind.fill(expect_sys,expect_ex1) %>% mutate(FAEVINTX='') %>%
...
testthat::test_that('derive_vars_merged_vaccine test case 4 - if the ex domain has multiple vaccinations at same visit',
                    {
                      ...
                      testthat::expect_warning(actual <- derive_vars_merged_vaccine(
                        dataset = face1,
                        dataset_ex = ex1,
                        dataset_supp = NULL,
                        dataset_suppex = NULL,
...
testthat::test_that("derive_vars_merged_vaccine test case 5 - merge the face and ex domain on records by USUBJID and FATPTREF if the FATPTREF, FALAT, FADIR and FALOC variables are not in the face domain ",
                    {
                      ...
                      expect_ad <- face1 %>% mutate(FATPTREF='VACCINATION 1',
                                                    FALAT='',
                                                     ...
                                                     FALOC='') %>%
                        filter(FASCAT!='SYSTEMIC') %>%
                        left_join(ex1 %>% mutate(EXTPTREF='VACCINATION 1',
                                                 EXLAT='',
                                                 EXDIR='',
                                                 EXLOC=''), by=c('USUBJID'='USUBJID','FATPTREF'='EXTPTREF',
